...
def _derivative_inplace(state, acc_func, out_k):
    """Zero-allocation derivative. Writes directly into pre-allocated out_k."""
    # Slicing creates a view (no allocation)
    r = state[:, :3]
    v = state[:, 3:]
...
def rk4_step_hardened(state, dt, acc_func, buffers):
    ...
    np.add(k1, k4, out=k1)
    np.add(k1, k2, out=temp) 
    np.multiply(temp, dt / 6.0, out=temp)
    np.add(state, temp, out=state)